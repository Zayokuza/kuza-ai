...
Kuza-v2 supports personalizing the underlying model using your own interaction history. Heavy training runs off-device on Google Colab (free tier), while your phone handles only lightweight data export and model file management.
...
kuza2 --finetune --ft-days 60 --ft-quality 0.6 --ft-model 7b
```
...
| `--ft-output` | `~/Downloads/kuza-finetune` | Output directory |

**Output files:**

- `kuza-finetune-0.5b.jsonl` — Dataset for the 0.5B model
- `kuza-finetune-7b.jsonl` — Dataset for the 7B model
- `kuza-finetune-qwen-coder-0.5b.ipynb` — Colab notebook
- `kuza-finetune-qwen-coder-7b.ipynb` — Colab notebook
...
2. Upload the generated notebook (`kuza-finetune-*.ipynb`).
3. Run all cells. Free T4 GPU takes 1–4 hours depending on model size.
4. Download `kuza-lora-adapter.zip` when training completes.
...
# Import to Kuza-v2
kuza2 --import-lora /path/to/kuza-lora-adapter --lora-model primary
```
...
```bash
# Merge on import
kuza2 --import-lora /path/to/adapter --lora-model primary --lora-merge
...
```bash
kuza2 --rollback --lora-model primary
```
